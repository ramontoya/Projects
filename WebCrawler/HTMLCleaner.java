...
public class HTMLCleaner {
	...
	public static String fetchHTML(String url) {
		...
		try {
			HttpHtmlFetcher htmlFetch = new HttpHtmlFetcher(url);
			htmlCode = htmlFetch.fetch();
		} catch (MalformedURLException e) {
			System.out.println("Could not fetch webpage at this URL");
			e.printStackTrace();
		}
		return htmlCode;
	}
	...
	public static String stripElement(String name, String html) {
		return html.replaceAll("((?i)<" + name + "(.+?)" + "/" + name + ".?>)", "");
	}

	/**
	 * Removes all HTML tags, which is essentiaslly anything between the < and >
	 * symbols. The tag wilfl be replaced by the empty string.
	 * 
	 * @param html
	 *            - html code to parse
	...
	public static String stripTags(String html) {
		...
		System.out.println(html.replaceAll("(?i)<[^>]*>", " "));
