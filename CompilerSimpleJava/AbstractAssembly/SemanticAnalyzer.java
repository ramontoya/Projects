...
public class SemanticAnalyzer implements ASTVisitor {
	...
	public Object VisitFunctionCallExpression(
		...
		if (func == null)
			...
			return new TypeTree(func.result(), assembly.callExpression(
					actualsTrees,functions.find(functioncall.name()).startlabel()));
		}
	...
	public Object VisitFunctionDefinitions(
		...
		AATStatement sequence = (AATStatement) functiondefinitions.elementAt(
				0).Accept(this);

		for (int j = 1; j < functiondefinitions.size(); j++)
			sequence = new AATSequential(sequence,
					(AATStatement) functiondefinitions.elementAt(j).Accept(
							this));
	...
	public Object VisitIntegerLiteral(ASTIntegerLiteral literal) {
		...
		System.out.println("Visiting Integer Literal" + literal.value());
	...
	public Object VisitNewClassExpression(ASTNewClassExpression newclass) {
		...
		return new TypeTree(classs, assembly.allocate(assembly
				.operatorExpression(
						assembly.constantExpression(classs.variables().size()),
						assembly.constantExpression(4), AATOperator.MULTIPLY)));
	}
	...
	public Object VisitUnaryOperatorExpression(
		...
		return new TypeTree(BooleanType.instance(), assembly.operatorExpression(exp.tree(), null, AATOperator.NOT));
	...
	public Object VisitVariableDefStatement(ASTVariableDefStatement vardef) {
		...
		if (initialize != null) {
			...
			return assembly.assignmentStatement(
					assembly.baseVariable(variables.find(vardef.name()).offset()),
					initial.tree());
		}
